...
var WasmBuffer = class {
   ...
   */
  constructor(exports2, size, dtype) {
...
var NDWasmImage = {
  ...
  decode(imageBytes) {
    ...
    const exports2 = ndwasm_default.runtime.exports;
    let imagePtr = 0;
    let resultPtr = 0;
    let pixelDataPtr = 0;
    try {
      imagePtr = exports2.malloc(imageBytes.length);
      if (!imagePtr) throw new Error("WASM malloc failed for image bytes.");
      new Uint8Array(exports2.mem.buffer, imagePtr, imageBytes.length).set(imageBytes);
      resultPtr = exports2.decode_image(imagePtr, imageBytes.length);
      if (!resultPtr) {
        console.error("Failed to decode image in WASM. The image format may be invalid or unsupported.");
        return null;
      }
      const resultView = new DataView(exports2.mem.buffer, resultPtr, 16);
      ...
      const pixelData = new Uint8ClampedArray(exports2.mem.buffer, pixelDataPtr, pixelDataSize).slice();
      return new NDArray(pixelData, { shape: [height, width, 4], dtype: "uint8c" });
    } finally {
      if (imagePtr) exports2.free(imagePtr, imageBytes.length);
      if (resultPtr) exports2.free(resultPtr, 16);
      if (pixelDataPtr) {
        const resultView = new DataView(exports2.mem.buffer, resultPtr, 16);
        const pixelDataSize = resultView.getUint32(4, true);
        exports2.free(pixelDataPtr, pixelDataSize);
      }
    }
  },
  /**
  ...
  encode(ndarray, { format = "png", quality = 90 } = {}) {
    ...
    try {
      pixelDataPtr = exports2.malloc(sourceData.byteLength);
      if (!pixelDataPtr) throw new Error("WASM malloc failed for pixel data.");
      new Uint8Array(exports2.mem.buffer, pixelDataPtr, sourceData.byteLength).set(sourceData);
      formatPtr = exports2.malloc(formatBytes.length);
      if (!formatPtr) throw new Error("WASM malloc failed for format string.");
      new Uint8Array(exports2.mem.buffer, formatPtr, formatBytes.length).set(formatBytes);
      resultPtr = exports2.encode_image(pixelDataPtr, width, height, channels, quality, formatPtr, formatBytes.length);
      if (!resultPtr) {
        console.error(`Failed to encode image to ${format} in WASM.`);
        return null;
      }
      const resultView = new DataView(exports2.mem.buffer, resultPtr, 8);
      ...
      const encodedData = new Uint8Array(exports2.mem.buffer, encodedDataPtr, encodedDataSize).slice();
      return encodedData;
    } finally {
      if (pixelDataPtr) exports2.free(pixelDataPtr, sourceData.byteLength);
      if (formatPtr) exports2.free(formatPtr, formatBytes.length);
      if (resultPtr) exports2.free(resultPtr, 8);
      if (encodedDataPtr) {
        const resultView = new DataView(exports2.mem.buffer, resultPtr, 8);
        const encodedDataSize = resultView.getUint32(4, true);
        exports2.free(encodedDataPtr, encodedDataSize);
      }
    }
  },
  /**
